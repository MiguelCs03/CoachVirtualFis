"""Serializadores para datos de entrenamiento de poses."""
...
class PoseTrainingDataSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            ...
            "tipo",
            "landmarks",
            "angulos",  # Para snapshots
            "frames",
            "duracion_segundos",
            "fps",
            "total_frames",  # Para secuencias
            "etiqueta",
            "created_at",
        ]
        read_only_fields = ["id", "created_at"]

    def validate_etiqueta(self, value: str) -> str:
    ...
    def validate(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if tipo == "snapshot":
            if not data.get("landmarks") or not data.get("angulos"):
                raise serializers.ValidationError("Los snapshots deben incluir 'landmarks' y 'angulos'")
        elif tipo == "secuencia":
            if not data.get("frames"):
                raise serializers.ValidationError("Las secuencias deben incluir 'frames'")
            if data.get("total_frames") and len(data["frames"]) != data["total_frames"]:
                raise serializers.ValidationError("El número de frames no coincide con total_frames")
