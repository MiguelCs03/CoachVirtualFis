...
"""Controladores para planes y suscripciones del usuario."""
...
class ListarPlanesVista(APIView):
    ...
    def get(self, request: Request) -> Response:
        ...
        return Response({"planes": PLANES, "mensaje": "Planes disponibles"})
...
class ComprarPlanVista(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        return Response(
            {
                "mensaje": "Suscripción creada. Pendiente de confirmación de pago.",
                "plan": plan_key,
...
class ConfirmarPagoVista(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        if historial.estado_pago == "confirmado":
            return Response({"mensaje": "El pago ya fue confirmado anteriormente", "plan": historial.plan})

        # Desactivar suscripciones anteriores del usuario
        HistorialSuscripcion.objects.filter(usuario=request.user, activo=True).update(activo=False)
...
class HistorialSuscripcionesVista(APIView):
    ...
    def get(self, request: Request) -> Response:
        ...
        historial = HistorialSuscripcion.objects.filter(usuario=usuario).order_by("-fecha_inicio")
        ...
        return Response({"historial": historial_data, "total": len(historial_data)})
...
class CancelarPlanVista(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        suscripcion_activa = HistorialSuscripcion.objects.filter(usuario=usuario, activo=True).first()
        ...
        if inmediato:
            ...
            mensaje = (
                "Plan marcado para cancelación. Se desactivará el "
                f"{suscripcion_activa.fecha_expiracion.strftime('%d/%m/%Y')}"
            )
        ...
        return Response(
            {
                ...
                "fecha_expiracion": usuario.fecha_expiracion_plan.isoformat()
