...
from .local_fitness import LocalFitnessSimulator, get_local_fitness_stats
...
def googlefit_stats(request: Request) -> Response:
    """
    ...
    try:
        ...
        logger.exception("Error en simulador local: %s", e)
        return Response(
            {"error": f"Error obteniendo estadísticas: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )


@api_view(["POST"])
@permission_classes([AllowAny])
def record_exercise(request: Request) -> Response:
    """
    ...
    try:
        ...
        logger.exception("Error registrando ejercicio: %s", e)
