...
export default function UniversalExercisePage() {
  ...
  const handleSetComplete = useCallback(() => {
    ...
    if (setCount < targetSets) {
      ...
      if (voiceEnabled)
        speak('¡Objetivo cumplido! Protocolo de entrenamiento completado.', 'encouragement', true)
      setIsActive(false)
    }
    ...
    (command) => {
      switch (command) {
        ...
        case 'RESUME':
          ...
          break
        case 'HELP':
          if (voiceEnabled) speak('Diga: Demostración, Pausa, Continuar o Descanso.', 'info')
          break
      }
    },
    [voiceEnabled]
  ...
  const toggleMic = () => {
    if (micEnabled) {
      ...
      startListening()
      setMicEnabled(true)
      if (voiceEnabled) speak('Micrófono activo. Control de voz habilitado.', 'info')
    }
  }
  ...
  const handlePoseDetected = useCallback(
    (landmarks) => {
      ...
      if (formCorrect) {
        ...
        setIsCorrect(true)
        setCorrections([])
        setCurrentInstruction('POSTURA_ÓPTIMA: Continúe')
      } else {
        ...
        setCurrentInstruction(newCorrections[0]?.message || 'AJUSTE_REQUERIDO')
      }
    },
      ...
      <div className="bg-[#0f0f0f] border-b border-white/10 px-6 py-4 fixed top-16 left-0 right-0 z-40 backdrop-blur-md">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex flex-col items-center">
            <span className="text-yellow-400 font-mono text-[9px] tracking-[0.4em] uppercase">
              MÓDULO_EJERCICIO_IA
            </span>
            <h1 className="text-xl font-black italic uppercase italic tracking-tighter leading-none">
              {exerciseData.nombre}
            </h1>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              className={`p-2 border transition-all ${voiceEnabled ? 'border-yellow-400 text-yellow-400 bg-yellow-400/5' : 'border-white/10 text-white/20'}`}
            >
            ...
            {isVoiceRecognitionSupported() && (
              <button
                onClick={toggleMic}
                className={`p-2 border transition-all ${micEnabled ? 'border-yellow-400 animate-pulse text-yellow-400' : 'border-white/10 text-white/20'}`}
              >
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <AnimatePresence>
              {isResting && (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  className="bg-yellow-400 text-black p-8 text-center space-y-4 shadow-[15px_15px_0px_rgba(255,230,0,0.1)]"
                >
                  <p className="text-[10px] font-black tracking-[0.5em] uppercase">
                    CRONÓMETRO_RECARGA_ATLETA
                  </p>
                  ...
                  <div className="flex items-center justify-center gap-4 pt-4">
                    <div className="text-[10px] font-mono font-bold uppercase tracking-widest">
                      SERIE_{setCount} / {exerciseDesc.series}
                    </div>
                    <button
                      onClick={handleRestComplete}
                      className="bg-black text-white px-6 py-2 text-[9px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all"
                    >
                      ABORTAR_DESCANSO
                    </button>
                  </div>
              ...
              <div className="bg-[#0f0f0f] border border-white/5 overflow-hidden">
                <div className="h-96 relative bg-black flex items-center justify-center p-8">
                  <img
                    ...
                    className="max-h-full filter grayscale brightness-75 contrast-125 transition-all duration-700 hover:grayscale-0 hover:brightness-100"
                  />
                  <div className="absolute bottom-6 right-6 border border-yellow-400/20 bg-black/60 px-4 py-2 flex items-center gap-2">
                    <Cpu className="w-4 h-4 text-yellow-400" />
                    <span className="text-[10px] font-black tracking-widest text-yellow-400">
                      DEMO_PROTOCOLO
                    </span>
                  </div>
                </div>
                <div className="p-10 space-y-8">
                  <div className="space-y-4 border-l-4 border-yellow-400 pl-8">
                    <h2 className="text-4xl font-black italic uppercase italic tracking-tighter leading-none">
                      {exerciseData.nombre}
                    </h2>
                    <p className="text-white/40 font-mono text-xs uppercase leading-relaxed tracking-tight">
                      {exerciseDesc.proposito}
                    </p>
                  </div>
                  ...
                  <div className="grid md:grid-cols-3 gap-6 pt-6 border-t border-white/5">
                    <StatBox
                      icon={Target}
                      label="MÚSCULOS"
                      value={exerciseDesc.musculos.join(', ').toUpperCase()}
                    />
                    <StatBox
                      icon={Zap}
                      label="META_SYNC"
                      value={`${exerciseDesc.reps} REPS × ${exerciseDesc.series} SERIES`}
                    />
                  ...
                  <div className="bg-white/[0.02] border border-white/5 p-8 space-y-4">
                    <h3 className="text-[10px] font-black text-yellow-400 tracking-[0.3em] uppercase flex items-center gap-3">
                      <BookOpen className="w-4 h-4" /> GUÍA_EJECUCIÓN_TÉCNICA
                    </h3>
                    <p className="text-sm font-mono text-white/60 leading-relaxed uppercase">
                      {exerciseDesc.instrucciones}
                    </p>
                  </div>
              ...
              <div className="relative bg-[#0f0f0f] border border-white/5 shadow-2xl">
                ...
                {exerciseData.url && (
                  <div
                    ...
                    className="absolute top-8 right-8 w-40 h-28 border border-white/20 bg-black/40 backdrop-blur-md p-1 group cursor-pointer hover:border-yellow-400/50 transition-all"
                  >
                    <img
                      src={exerciseData.url}
                      alt="Ref"
                    ...
                    />
                    <div className="absolute inset-x-0 bottom-0 py-1 bg-black/60 text-[8px] font-black text-center text-white/60 group-hover:text-yellow-400">
                      REFERENCIA_SYNC
                    </div>
                  </div>
                )}
            ...
            {isActive && (
              <div className="flex items-center justify-center gap-6 pt-6">
                ...
                <button
                  ...
                  className={`w-20 h-20 flex items-center justify-center rounded-full border-2 transition-all shadow-[0_0_20px_rgba(0,0,0,0.5)] ${isPaused ? 'bg-yellow-400 border-yellow-400 text-black' : 'bg-transparent border-white/20 text-white hover:border-yellow-400 hover:text-yellow-400'}`}
                >
                  {isPaused ? (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#0f0f0f] border border-white/5 p-8 text-center space-y-2 shadow-[10px_10px_0px_rgba(255,255,255,0.02)]">
              <span className="text-[8px] font-black text-white/20 tracking-[0.4em] uppercase">
                TELEMETRÍA_CRONO
              </span>
              <div className="text-5xl font-mono font-black italic tracking-tighter text-white">
                {formatTime(timer)}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-1">
              <div className="bg-[#0d0d0d] border border-white/5 p-6 text-center space-y-4">
                ...
                <div className="text-6xl font-black italic text-yellow-400 tracking-tighter">
                  {repCount}
                </div>
                <div className="text-[10px] font-mono text-white/10 uppercase">
                  DE {exerciseDesc.reps}
                </div>
              </div>
              <div className="bg-[#0d0d0d] border border-white/5 p-6 text-center space-y-4">
                <span className="text-[7px] font-black text-white/20 tracking-widest uppercase">
            ...
            <div className="bg-red-500/5 border-l-2 border-red-500 p-8 space-y-6">
              <h3 className="text-[9px] font-black text-red-500 tracking-[0.3em] uppercase flex items-center gap-3">
                <AlertCircle className="w-4 h-4" /> REPORTE_DE_FALLOS
              </h3>
              <ul className="space-y-3">
                {exerciseDesc.erroresComunes.map((err, i) => (
                  <li
                    key={i}
                    className="flex gap-3 text-[10px] font-mono text-white/40 uppercase leading-tight"
                  >
            ...
            <div className="bg-[#0f0f0f] border border-white/5 p-8 space-y-6">
              <h3 className="text-[9px] font-black text-white/20 tracking-[0.3em] uppercase border-b border-white/5 pb-2">
                ACCESO_RÁPIDO_SISTEMA
              </h3>
              <div className="grid grid-cols-1 gap-4">
                <TerminalBtn onClick={() => setShowDemo(true)} icon={Eye} text="REPRODUCIR_DEMO" />
                <TerminalBtn
                  onClick={() => voiceEnabled && speak(exerciseDesc.instrucciones, 'info', true)}
                  icon={Volume2}
                  text="ESCUCHAR_TUTORIAL"
                />
                <TerminalBtn
      ...
      <AnimatePresence>
        {showDemo && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/95 backdrop-blur-xl flex items-center justify-center z-[100] cursor-pointer"
          ...
          >
            <div className="bg-[#0f0f0f] border border-white/10 p-1 bg-black max-w-4xl w-full mx-4 shadow-[0_0_100px_rgba(255,230,0,0.1)] relative">
              ...
              <img
                src={exerciseData.url}
                alt={exerciseData.nombre}
                className="w-full h-auto max-h-[70vh] object-contain filter contrast-125"
              />
              <div className="p-8 text-center bg-[#0d0d0d] space-y-2">
                <h4 className="text-2xl font-black uppercase italic italic tracking-tighter text-white">
                  {exerciseData.nombre}
                </h4>
                <p className="text-[10px] font-mono text-white/20 uppercase tracking-widest">
                  PRESIONE EN CUALQUIER LUGAR PARA ABORTAR VISTA
                </p>
              </div>
            </div>
          </motion.div>
            ...
            <div className="text-center space-y-8 max-w-md mx-auto p-12 bg-[#0d0d0d] border border-white/10 shadow-[20px_20px_0px_rgba(255,230,0,0.05)]">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[10px] font-mono text-white/40 uppercase tracking-widest leading-relaxed">
                  BIOMETRÍA EN PAUSA. EL SISTEMA ESPERA LA SEÑAL DE REANUDACIÓN PARA CONTINUAR EL
                  PROCESAMIENTO.
                </p>
              </div>
              <button
                onClick={handleTogglePause}
                className="w-full bg-white text-black py-4 font-black uppercase text-[12px] tracking-[0.4em] hover:bg-yellow-400 transition-all"
              >
                REANUDAR_PROTOCOLO
              </button>
            </div>
...
const ControlCircle = ({ onClick, icon: Icon, label }) => (
  <div className="flex flex-col items-center gap-2 group">
    <button
      onClick={onClick}
      className="p-4 bg-[#1a1a1a] border border-white/10 rounded-full hover:border-yellow-400/50 hover:bg-white/5 transition-all text-white/40 hover:text-white"
    >
      <Icon className="w-6 h-6" />
    </button>
    <span className="text-[7px] font-black tracking-[0.2em] text-white/10 group-hover:text-yellow-400 transition-colors uppercase">
      {label}
    </span>
  </div>
)
...
const TerminalBtn = ({ onClick, icon: Icon, text }) => (
  <button onClick={onClick} className="flex items-center gap-4 group text-left">
    ...
    <span className="text-[9px] font-black tracking-widest text-white/40 group-hover:text-white transition-colors uppercase">
      {text}
    </span>
  </button>
)
