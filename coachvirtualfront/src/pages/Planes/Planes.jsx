  ...
  useEffect(() => {
    const cargarPlanes = async () => {
      try {
        ...
        setPlanes([
          { id: 1, clave: 'gratis', nombre: 'FREE_TERMINAL', precio: 0, icono: '🆓', color: 'border-white/10', minutos_por_dia: 15, feedback_voz: false, analisis_angulos: false, historial_dias: 0, con_anuncios: true, popular: false },
          { id: 2, clave: 'basico', nombre: 'BASIC_UNIT', precio: 25, icono: '⭐', color: 'border-yellow-400/20', minutos_por_dia: 60, feedback_voz: true, analisis_angulos: false, historial_dias: 7, con_anuncios: true, popular: true },
          { id: 3, clave: 'premium', nombre: 'PREMIUM_ELITE', precio: 49, icono: '👑', color: 'border-yellow-400', minutos_por_dia: -1, feedback_voz: true, analisis_angulos: true, historial_dias: -1, con_anuncios: false, popular: false },
        ]);
      } finally {
  ...
  if (loadingPlanes) {
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-[10px] font-mono tracking-[0.5em] text-white/20 uppercase">CARGANDO_SUSCRIPCIONES_BIOS</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#050505] p-6 lg:p-12 text-white font-sans">
      <div className="max-w-7xl mx-auto space-y-16">
        ...
        <div className="text-center space-y-6">
          ...
          <h1 className="text-5xl md:text-7xl font-black italic uppercase italic tracking-tighter block leading-tight">ELIGE TU <span className="text-yellow-400">PLAN</span></h1>
          <p className="text-white/20 font-mono text-[9px] tracking-[0.3em] uppercase max-w-lg mx-auto leading-relaxed">SINCRONICE SU NIVEL DE PROCESAMIENTO BIOMECÁNICO PARA DESBLOQUEAR MÓDULOS AVANZADOS DE ENTRENAMIENTO.</p>
        </div>
        ...
        <div className="flex flex-col items-center">
            <div className="bg-[#0f0f0f] border border-white/5 p-8 flex flex-col items-center text-center space-y-4 shadow-[15px_15px_0px_rgba(255,230,0,0.05)] w-full max-w-md">
               <p className="text-[8px] font-black text-white/20 tracking-[0.4em] uppercase">ARCHIVO_ACTIVO_DEL_ATLETA</p>
               <h3 className="text-2xl font-black italic uppercase italic tracking-tighter text-white">{(planes.find(p => p.clave === currentPlanKey)?.nombre || 'FREE_TERMINAL').toUpperCase()}</h3>
            ...
            return (
              <motion.div
                ...
                className={cx(
                    ...
                    plan.popular ? "border-yellow-400" : isCurrentPlan ? "border-green-500/40" : "border-white/5"
                )}
              >
                ...
                <div className="p-10 space-y-10 flex-1 flex flex-col justify-between">
                  ...
                  <div className="space-y-6 text-center">
                    <div className="text-6xl group-hover:scale-110 transition-transform duration-500">{plan.icono}</div>
                    <div>
                        <h3 className="text-2xl font-black italic uppercase italic tracking-tighter text-white">{plan.nombre.toUpperCase()}</h3>
                        <p className="text-[10px] font-mono text-white/20 uppercase tracking-widest mt-1">SYST_MODULE_{plan.id}</p>
                    </div>
                    <div className="inline-block p-4 bg-white/[0.02] border border-white/5">
                      ...
                      {plan.precio > 0 && <span className="text-white/20 font-mono text-[9px] ml-2">/MENSUAL</span>}
                    </div>
                  </div>

                  {/* Lista de Características del Sistema */}
                  <div className="space-y-4">
                     <p className="text-[8px] font-black text-white/20 tracking-widest uppercase mb-4 border-b border-white/5 pb-2">CARACTERÍSTICAS_TÉCNICAS</p>
                     <ul className="space-y-3">
                        <FeatureItem 
                            active={true} 
                            text={plan.minutos_por_dia === -1 ? 'TIEMPO_ILIMITADO_CORE' : `${plan.minutos_por_dia}_MIN_DIARIOS`} 
                        />
                        <FeatureItem 
                            active={plan.feedback_voz} 
                            text="RETROALIMENTACIÓN_VOZ_IA" 
                        />
                        <FeatureItem 
                            active={plan.analisis_angulos} 
                            text="ANÁLISIS_CINEMÁTICO_ANG" 
                        />
                        <FeatureItem 
                            active={!plan.con_anuncios} 
                            text={plan.con_anuncios ? 'PUBLICIDAD_SISTEMA_ACTIVA' : 'PROTOCOLOS_SIN_INTERRUPCIÓN'} 
                        />
                     </ul>
                  </div>
                  ...
                  <div className="pt-6">
                    {isCurrentPlan ? (
                        <button disabled className="w-full bg-green-500/10 border border-green-500/20 text-green-500 py-4 font-black uppercase text-[10px] tracking-widest italic cursor-not-allowed">✓ SINCRONIZADO</button>
                    ) : (
                        <button
                          ...
                          className={cx(
                              ...
                              isGratis ? "bg-white/[0.05] text-white/20 cursor-not-allowed" : "bg-white text-black hover:bg-yellow-400 shadow-sm"
                          )}
                        >
                          {isLoading ? (
                            <><Loader2 className="w-4 h-4 animate-spin" /> CONECTANDO_PASARELA...</>
                          ) : (
                            isGratis ? "NIVEL_BASE_INICIADO" : <><CreditCard className="w-4 h-4" /> ACTIVAR_PASARELA</>
                          )}
                        </button>
                    )}
                  </div>
...
const FeatureItem = ({ active, text }) => (
  <li className="flex items-center gap-3 group/item">
    <div className={cx(
        ...
        active ? "border-yellow-400/30 text-yellow-400 bg-yellow-400/5" : "border-white/5 text-white/10"
    )}>
...
const FooterBadge = ({ icon: Icon, text }) => (
    <div className="flex items-center gap-4 justify-center md:justify-start group">
        ...
        <span className="text-[9px] font-black tracking-widest text-white/40 uppercase max-w-[180px] leading-relaxed">{text}</span>
    </div>
);
