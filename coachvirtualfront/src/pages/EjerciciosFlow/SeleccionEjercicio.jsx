...
export default function SeleccionEjercicio() {
  ...
  const handleSelectEjercicio = (ejercicio) => {
    ...
    if (isFisioterapia) {
      ...
      if (nombreNorm.includes('piern') || nombreNorm.includes('glute') || nombreNorm.includes('puente') || nombreNorm.includes('sentad') || nombreNorm.includes('talon') || nombreNorm.includes('punta')) {
        ...
        if (nombreNorm.includes('glute') || (nombreNorm.includes('elev') && nombreNorm.includes('glute'))) {
    ...
    if (nombreNorm.includes('flexion')) {
        if (isPectoral) navigate('/categoria/gimnasio/pectorales/flexiones', { state: { imageUrl: ejercicio.url, nombre: ejercicio.nombre } });
        else navigate('/categoria/gimnasio/brazos/flexiones', { state: { imageUrl: ejercicio.url, nombre: ejercicio.nombre } });
        return;
    }
    if (nombreNorm.includes('press') && nombreNorm.includes('banca')) {
        if (isPectoral) navigate('/categoria/gimnasio/pectorales/press-banca', { state: { imageUrl: ejercicio.url, nombre: ejercicio.nombre } });
        else navigate('/categoria/gimnasio/brazos/press-banca', { state: { imageUrl: ejercicio.url, nombre: ejercicio.nombre } });
        return;
    }
    ...
    navigate(`/workout/exercise/${ejercicio.id}`, {
      state: { imageUrl: ejercicio.url, nombre: ejercicio.nombre, porcentaje: ejercicio.porcentaje }
    });
  };
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="space-y-4 border-l-4 border-yellow-400 pl-8">
           <div className="flex items-center gap-3 font-mono text-[9px] text-white/20 tracking-[0.4em]">
              <Cpu className="w-3 h-3 text-yellow-400" /> {breadcrumb.categoria.toUpperCase()} / {breadcrumb.parte.toUpperCase()}
           </div>
           <h1 className="text-5xl font-black italic uppercase italic tracking-tighter">PROTOCOLO DE <span className="text-yellow-400">{breadcrumb.parte}</span></h1>
           <p className="text-white/40 font-mono text-[10px] tracking-widest uppercase">FILTRADO: SELECCIONE EL MÓDULO OPERATIVO PARA INICIAR EL PROCESAMIENTO BIOMECÁNICO.</p>
        </div>
        ...
        {loading && (
          <div className="py-20 flex flex-col items-center gap-4">
             ...
             <span className="text-[9px] font-mono tracking-[0.5em] text-white/20 animate-pulse">CARGANDO_BIOS...</span>
          </div>
        )}
        ...
        {error && !loading && (
          <div className="bg-red-500/5 border-l-2 border-red-500 p-8 space-y-4">
            ...
            <button onClick={fetchEjercicios} className="px-6 py-2 border border-red-500/50 text-red-500 hover:bg-red-500 hover:text-white transition-all text-xs font-black uppercase tracking-widest">REINTENTAR_SYNC</button>
          </div>
        )}
        ...
        {!loading && !error && ejercicios.length === 0 && (
          <div className="bg-white/[0.02] border border-white/5 p-12 text-center space-y-4">
            ...
            <p className="text-white/40 font-mono text-xs uppercase tracking-widest">NULA_DISPONIBILIDAD: NO HAY MÓDULOS ACTIVOS PARA ESTA CONFIGURACIÓN.</p>
          </div>
        )}
              ...
              >
                ...
                <div className="h-48 relative overflow-hidden bg-black/40">
                  {ejercicio.url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-white/10"><Activity className="w-12 h-12" /></div>
                  )}
                  <div className="absolute top-4 left-4 border border-yellow-400/20 bg-black/60 px-2 py-1 flex items-center gap-2">
                     <span className="text-[8px] font-black tracking-widest text-yellow-400">0{idx + 1}</span>
                  </div>
                </div>
                ...
                <div className="p-6 space-y-6 flex-1 flex flex-col">
                  <div className="flex-1 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                      <div className="flex flex-col gap-1">
                        <span className="text-[8px] font-black text-white/20 tracking-widest uppercase">TIEMPO_EST</span>
                        <div className="flex items-center gap-2 text-[10px] font-mono"><Clock className="w-3 h-3 text-yellow-400" /> {ejercicio.duracion.toUpperCase()}</div>
                      </div>
                      <div className="flex flex-col gap-1">
                        <span className="text-[8px] font-black text-white/20 tracking-widest uppercase">QUEMA_CAL</span>
                        <div className="flex items-center gap-2 text-[10px] font-mono"><Zap className="w-3 h-3 text-yellow-400" /> {ejercicio.calorias.toUpperCase()}</div>
                      </div>
                    </div>

                    <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                       <span className="text-[8px] font-black text-white/20 tracking-widest uppercase">NIVEL_REQUERIDO</span>
                       <span className={cx(
                         "text-[9px] font-black uppercase tracking-widest px-2 py-1 border",
                         ejercicio.dificultad === 'Principiante' ? 'border-green-500/20 text-green-500' :
                         ejercicio.dificultad === 'Intermedio' ? 'border-yellow-400/20 text-yellow-400' :
                  ...
                  >
                    INICIAR <ChevronRight className="w-4 h-4 group-hover/btn:translate-x-1 transition-transform" />
                  </button>
                </div>
              </motion.div>
            ))}
