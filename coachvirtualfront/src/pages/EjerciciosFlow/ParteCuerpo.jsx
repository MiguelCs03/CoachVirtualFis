...
export default function ParteCuerpo() {
  ...
  const fetchPartesCuerpo = async () => {
    try {
      ...
      const musculosFiltrados = musculosAll.filter((m) => getTipoId(m) === categoriaId)
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="space-y-4 border-l-4 border-yellow-400 pl-8">
          <div className="flex items-center gap-3 font-mono text-[9px] text-white/20 tracking-[0.4em]">
            ...
            {selectedCategoria.toUpperCase() || 'CARGANDO_BIOS...'}
          </div>
          <h1 className="text-5xl font-black italic uppercase italic tracking-tighter leading-none">
            SELECCIONE LA <span className="text-yellow-400">ZONA_MUSCULAR</span>
          </h1>
          <p className="text-white/40 font-mono text-[10px] tracking-widest uppercase max-w-lg">
            FILTRE EL PROTOCOLO DE ENTRENAMIENTO POR UNIDAD BIOMECÁNICA ACTIVA.
          </p>
        </div>
        ...
        <div className="flex gap-1 h-1 w-full bg-white/5 overflow-hidden">
          <div
            className="w-full bg-yellow-400 animate-pulse transition-all duration-1000"
            style={{ width: '40%' }}
          ></div>
        </div>
        ...
        {error && !loading && (
          <div className="bg-red-500/5 border-l-2 border-red-500 p-8 space-y-4">
            <h3 className="text-red-500 font-black uppercase text-xs tracking-widest flex items-center gap-3">
              <Shield className="w-4 h-4" /> REPORTE_DE_FALLO_OPERATIVO
            </h3>
            <p className="text-red-400 font-mono text-[10px] uppercase tracking-tight">{error}</p>
            <button
              onClick={fetchPartesCuerpo}
              className="px-6 py-2 border border-red-500/50 text-red-500 hover:bg-red-500 hover:text-white transition-all text-[9px] font-black uppercase tracking-widest"
            >
              REINTENTAR_SYNC
            </button>
          </div>
        ...
        {!loading && !error && partesCuerpo.length === 0 && (
          <div className="bg-[#0f0f0f] border border-white/5 p-16 text-center space-y-6">
            ...
            <p className="text-white/40 font-mono text-[10px] uppercase tracking-widest">
              ERROR_DE_MÓDULO: NO HAY UNIDADES ACTIVAS PARA ESTA CATEGORÍA.
            </p>
          </div>
        )}
                ...
                >
                  ...
                  <div className="p-8 space-y-4">
                    <p className="text-[9px] font-mono text-white/40 uppercase tracking-tight leading-relaxed">
                      ACTIVA EL PROTOCOLO DE ENTRENAMIENTO ESPECÍFICO PARA LA ZONA DE{' '}
                      {parte.nombre.toUpperCase()}.
                    </p>

                    <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                      <span className="text-[8px] font-black text-white/20 tracking-[0.3em] uppercase italic group-hover:text-yellow-400/40 transition-colors">
                        EXPLORAR_CATÁLOGO
                      </span>
