...
class Usuario extends Component {
  ...
  loadList = async () => {
    ...
    try {
      ...
      const msg =
        err?.response?.data?.detail || err?.message || 'No se pudo cargar la lista de usuarios.'
  ...
  validate = () => {
    ...
    if (!this.state.isEditing && !password?.trim()) errors.password = 'Contraseña requerida'
      ...
      <main className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-900 to-purple-900 p-4 sm:p-6">
        ...
        <section className="bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl p-4 sm:p-6 w-full max-w-3xl mx-auto border border-white/20 mb-8">
          ...
          <form className="grid grid-cols-1 gap-4" onSubmit={this.handleSubmit}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              {this.renderField('Fecha expiración del plan', 'fecha_expiracion_plan', 'date')}
            </div>
        ...
        <section className="bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl p-4 sm:p-6 w-full max-w-6xl mx-auto border border-white/20">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-3">
            <h2 className="text-lg sm:text-xl font-semibold text-white">Listado de usuarios</h2>
            <button
                    ...
                    {paged.map((row) => {
                      ...
                      return (
                        <tr key={row.id} className="border-b border-white/10">
                          ...
                          {/* datos de plan */}
                          <td className="py-2 pr-4">{row.plan_actual || 'Sin plan'}</td>
                          <td className="py-2 pr-4">
                          ...
                          <td className="py-2 pr-4">{planActivo ? 'Sí' : 'No'}</td>
                          <td className="py-2 pr-4">{puedeEntrenar ? 'Sí' : 'No'}</td>
                          <td className="py-2 pr-4">
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-white font-semibold text-lg">{row.username}</p>
                      ...
                      <div className="text-white/80 text-sm space-y-1">
                        ...
                        <p>
                          <span className="font-medium">Género:</span> {row.genero || '-'}
                        </p>
                        <p>
                          <span className="font-medium">Altura:</span> {row.altura ?? '-'} m
                        </p>
                        <p>
                          <span className="font-medium">Peso:</span> {row.peso ?? '-'} kg
                        </p>
                        {/* datos de plan */}
                        <p>
                          <span className="font-medium">Plan:</span> {row.plan_actual || 'Sin plan'}
                        </p>
                        <p>
                        ...
                        <p>
                          <span className="font-medium">Estado plan:</span> {estadoPlan}
                        </p>
                        <p>
