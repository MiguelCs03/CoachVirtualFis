      ...
      <div className="max-w-4xl mx-auto space-y-12">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-end justify-between gap-8 border-b border-white/5 pb-10">
          <div className="flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                <p className="text-yellow-400 font-mono text-[10px] tracking-[0.4em] uppercase">
                  ARCHIVO_OPERATIVO_SISTEMA
                </p>
                <h1 className="text-4xl md:text-6xl font-black italic uppercase italic tracking-tighter leading-none">
                  {ui?.email.split('@')[0]}
                </h1>
              </div>
              <div className="flex items-center gap-4 text-white/40 text-[9px] font-black tracking-[0.2em] uppercase">
                <span className="flex items-center gap-2 border border-white/10 px-3 py-1 bg-white/[0.02]">
                  <Mail className="w-3 h-3" /> {ui?.email}
                </span>
                <span className="flex items-center gap-2 border border-yellow-400/20 px-3 py-1 bg-yellow-400/5 text-yellow-400">
                  ID://{ui?.id}
                </span>
              </div>
            </div>
          ...
          >
            {isEditing ? (
              <>
                <X className="w-4 h-4" /> CANCELAR_MOD
              </>
            ) : (
              <>
                <Edit3 className="w-4 h-4" /> EDITAR_PERFIL
              </>
            )}
          </button>
        </div>
          ...
          <div className="space-y-6">
            <div className="bg-[#0f0f0f] border border-white/5 p-8 space-y-8">
              <div className="space-y-2">
                <p className="text-[8px] font-black text-white/20 tracking-[0.3em] uppercase">
                  SISTEMA_DASHBOARD
                </p>
                <h3 className="text-xs font-black italic uppercase italic tracking-tighter text-yellow-400">
              ...
              <div className="space-y-4">
                <StatusItem
                  icon={Shield}
                  label="PLAN_ACTUAL"
                  value={ui?.planActual.toUpperCase()}
                  active
                />
                <StatusItem
                  icon={Calendar}
                  label="VENCIMIENTO"
                  value={ui?.fechaExpiracionPlan.toUpperCase()}
                />
                <StatusItem
                  icon={Zap}
                  label="ESTADO_ENTRENO"
                  value={ui?.puedeEntrenar ? 'OPERATIVO_SINC' : 'LIMITADO_BIOS'}
                  color={ui?.puedeEntrenar ? 'yellow' : 'red'}
                />
              </div>

              <div className="pt-8 border-t border-white/5 space-y-4">
                <p className="text-[8px] font-mono text-white/20 leading-relaxed uppercase tracking-tighter">
                  EL STATUS_OPERATIVO DEFINE LOS LÍMITES DE PROCESAMIENTO BIOMECÁNICO Y ACCESO A
                  MÓDULOS_IA DEL SISTEMA V-COACH.
                </p>
                <button
                  onClick={() => navigate('/planes')}
                  className="w-full border border-white/10 py-3 text-[9px] font-black uppercase tracking-widest hover:border-yellow-400 hover:text-yellow-400 transition-all"
                >
                  MEJORAR_SISTEMA
                </button>
              </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            {loading ? (
              ...
              <form
                ...
                className="bg-[#0f0f0f] border border-white/5 p-12 space-y-12"
              >
                ...
                <AnimatePresence>
                  {isEditing && (
                    <motion.div
                      ...
                      exit={{ opacity: 0 }}
                      className="pt-10 border-t border-white/5 flex gap-4"
                    >
                      <button
                        type="submit"
                        disabled={saving}
                        className="flex-1 bg-yellow-400 text-black py-5 font-black uppercase tracking-[0.3em] text-[10px] hover:bg-white transition-all flex items-center justify-center gap-3"
                      >
                        {saving ? (
...
const StepSection = ({ title, children }) => (
  <div className="space-y-6">
    <h4 className="text-[10px] font-black italic uppercase italic tracking-tighter text-white/60 mb-8 border-b border-white/5 pb-2">
      {title}
    </h4>
    {children}
  </div>
...
const StatusItem = ({ icon: Icon, label, value, active, color }) => (
  <div className="flex items-center gap-4">
    <div
      className={`p-2 border ${active ? 'border-yellow-400/20 text-yellow-400' : 'border-white/5 text-white/20'}`}
    >
    ...
    <div className="flex-1">
      ...
      <p
        className={`text-[9px] font-mono uppercase ${color === 'yellow' ? 'text-yellow-400' : color === 'red' ? 'text-red-500' : 'text-white/60'}`}
      >
        {value}
      </p>
    </div>
...
const InputField = ({ label, type, value, onChange, placeholder }) => (
  <div className="space-y-2">
    <label className="text-[9px] font-black text-white/30 tracking-widest uppercase ml-1">
      {label}
    </label>
    <input
      type={type}
