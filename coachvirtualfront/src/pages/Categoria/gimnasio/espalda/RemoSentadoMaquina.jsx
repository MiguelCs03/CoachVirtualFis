...
const StatusCard = ({ label, value, status, error, warning, neutral }) => (
  <div className={cx(
      ...
      warning ? "bg-yellow-400/10 border-yellow-400/40" : 
      neutral ? "bg-white/[0.03] border-white/10" : 
      "bg-green-500/10 border-green-500/40"
  )}>
      <span className="text-[7px] font-black tracking-widest text-white/30 uppercase text-center leading-none mb-1">{label}</span>
      <div className={cx(
      ...
      <span className={cx(
          ...
          error ? "border-red-500/20 text-red-500 bg-red-500/5" : 
          warning ? "border-yellow-400/20 text-yellow-400 bg-yellow-400/5" : 
          neutral ? "border-white/10 text-white/40 uppercase" :
...
const CheckItem = ({ label, status, isWarning, pending, special }) => (
  <li className="flex items-center justify-between group">
    <span className="text-[10px] font-black text-white/40 group-hover:text-white transition-colors tracking-tight uppercase">{label}</span>
          ...
          <div className="grid lg:grid-cols-2 gap-12 bg-[#0f0f0f] border border-white/5 overflow-hidden shadow-2xl relative">
            ...
            <div className="relative h-full min-h-[450px] bg-black group">
                ...
                <div className="absolute bottom-8 left-8 border border-yellow-400/20 bg-black/60 px-4 py-2 flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-black tracking-widest text-yellow-400 uppercase">MÓDULO_HIPERTROFIA_ESPALDA_v3</span>
                </div>
            </div>
            ...
            <div className="p-12 space-y-10 flex flex-col justify-between">
              <div className="space-y-8">
                 <div className="space-y-3">
                    ...
                    <h1 className="text-5xl font-black italic uppercase italic tracking-tighter leading-none">{passedNombre}</h1>
                    <p className="text-white/40 font-mono text-[10px] tracking-widest uppercase">MONITOREO BIOMÁTRICO DE INCLINACIÓN Y FLEXIÓN DE SEGURIDAD.</p>
                 </div>

                 <div className="space-y-6 pt-10 border-t border-white/5 list-none">
                    <InstructionItem number="01" text="POSICIÓN_SENTADA: Rodillas en semiflexión obligatoria." />
                    <InstructionItem number="02" text="EJE_COLUMNA: Espalda neutra, evite balanceos cinéticos." />
                    <InstructionItem number="03" text="TRACCIÓN: Extienda el maneral hacia el plexo solar bajo." />
                 </div>
              </div>
              ...
              <button onClick={() => setStarted(true)} className="w-full bg-white text-black hover:bg-yellow-400 font-black py-6 transition-all flex items-center justify-center gap-4 uppercase text-[12px] tracking-[0.4em] group">
                INICIAR_RUTINA_SYNC <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </button>
            </div>
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-8">
          ...
          <button onClick={() => setStarted(false)} className="px-8 py-3 bg-red-500/5 border border-red-500/20 text-red-500 hover:bg-red-500 hover:text-white transition-all text-[10px] font-black uppercase tracking-widest flex items-center gap-3 group">
            <X className="w-4 h-4 group-hover:scale-125 transition-transform" /> FINALIZAR_Y_REPORTAR
          </button>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2 space-y-8">
                <div className="bg-[#0f0f0f] border border-white/5 relative overflow-hidden shadow-2xl">
                    ...
                    <AnimatePresence>
                      <motion.div 
                        ...
                        className={cx(
                            ...
                            feedback.includes('⚠️') ? 'bg-red-500 border-red-500 text-white animate-pulse' : 'bg-black/60 border-yellow-400/20 text-yellow-400'
                        )}
                      >
                         <div className="flex items-center gap-4">
                            {feedback.includes('⚠️') ? <ShieldAlert className="w-5 h-5" /> : <Zap className="w-5 h-5" />}
                            <span className="text-[12px] font-black tracking-[0.3em] uppercase">{feedback}</span>
                         </div>
                      </motion.div>
                    </AnimatePresence>
                    ...
                    <div className="absolute top-8 left-8 flex flex-col gap-2">
                        <div className="flex items-center gap-3 bg-black/80 border border-white/10 px-4 py-1.5">
                            ...
                            <span className="text-[9px] font-mono font-black tracking-widest text-white/40 uppercase">V-LINK_ACTIVE</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-[#0f0f0f] border border-white/5 p-8 space-y-6">
                   <div className="flex items-center justify-between border-b border-white/5 pb-4">
                      ...
                      <div className="text-[8px] font-mono text-white/10">ANALYZING_CORE_STABILITY...</div>
                   </div>
                   ...
                   <div className="grid grid-cols-3 gap-6">
                       ...
                       <StatusCard 
                          ...
                          status={currentAngles.knee > KNEE_LOCK_THRESHOLD ? '⚠️ ALERTA_BLOQUEO' : '✅ ÓPTIMO'}
            ...
            <div className="space-y-8">
                ...
                <div className="bg-[#0f0f0f] border border-white/5 p-12 text-center space-y-8 shadow-[20px_20px_0px_rgba(255,230,0,0.03)] relative overflow-hidden">
                    <h2 className="text-[10px] font-black text-white/20 tracking-[0.5em] uppercase">CONTEO_REPETICIONES</h2>
                    <div className="text-[140px] font-black italic text-white tracking-tighter leading-none">{repCount}</div>
                    
                    <div className="space-y-4 pt-4 border-t border-white/5">
                        <div className="relative h-2 w-full bg-white/5 overflow-hidden">
                            <motion.div 
                               animate={{ width: stage === 'pulling' ? '50%' : stage === 'squeezing' ? '100%' : '0%' }}
                               className={cx(
                                   "absolute h-full transition-all duration-300",
                                   stage === 'squeezing' ? "bg-yellow-400 shadow-[0_0_15px_rgba(255,230,0,0.5)]" : "bg-white/40"
                               )} 
                            />
                        </div>
                        <p className="text-[9px] font-black text-yellow-400/40 tracking-[0.3em] uppercase italic italic italic">ESTADO_ACTUAL: {stage.replace('_', ' ')}</p>
                    </div>
                ...
                <div className="bg-[#111111] border-l-4 border-yellow-400 p-10 space-y-8 shadow-xl">
                    <div className="flex items-center gap-4 border-b border-white/5 pb-4">
                        ...
                        <h3 className="font-black text-xs tracking-widest uppercase">CHECKLIST_DE_SEGURIDAD</h3>
                    </div>
                    ...
                    <div className="bg-white/[0.02] border border-white/5 p-4 text-center">
                       <p className="text-[8px] font-mono text-white/30 uppercase leading-relaxed tracking-wider">Protocolo de seguridad gestionado por V-COACH IA Core v4.9</p>
                    </div>
                </div>
                ...
                <div className="bg-[#0f0f0f] border border-white/5 p-6 flex items-center justify-between group cursor-pointer hover:border-yellow-400/30 transition-all">
                    <div className="flex items-center gap-3">
                       ...
                       <span className="text-[10px] font-black tracking-widest uppercase text-white/40 group-hover:text-white">LOGS_DE_ESTADÍSTICAS</span>
                    </div>
