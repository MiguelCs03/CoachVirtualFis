          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-200">
            ...
            <div className="p-8">
              ...
              <ul className="list-disc pl-5 text-slate-600 mb-8 space-y-2 text-sm">
                <li>
                  Levanta un pie del suelo solo unos centímetros (como subiendo un escalón bajo).
                </li>
                <li>Todo el peso debe ir a la pierna que se queda en el suelo.</li>
                <li>
                  <strong>Evita que la cadera se hunda</strong> del lado que levantas.
                </li>
              </ul>
              <button
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden relative">
              ...
              <div className="absolute top-4 right-4 w-32 h-8 bg-black/30 backdrop-blur-md rounded-full border border-white/20 flex items-center justify-center relative overflow-hidden">
                <div
                  className={`absolute h-full w-full transition-opacity duration-300 ${metrics.isStable ? 'bg-green-500/20' : 'bg-red-500/20'}`}
                ></div>
                <div
                  className={`w-20 h-1 bg-white rounded transition-transform duration-300 origin-center`}
                  style={{
                    transform: `rotate(${metrics.isRightLeg ? metrics.pelvicTilt * 2 : -metrics.pelvicTilt * 2}deg)`,
                  }}
                ></div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex justify-center items-end gap-1 h-32">
              <div className="w-8 bg-slate-100 rounded-t relative h-full">
                ...
                {/* Pierna Izquierda */}
                <div
                  className={`absolute bottom-0 w-full bg-blue-400 transition-all duration-200 rounded-t
                            ${!metrics.isRightLeg ? 'opacity-100' : 'opacity-30'}`}
                  style={{
                    height: !metrics.isRightLeg
                      ? `${Math.min(100, (180 - metrics.activeHipAngle) * 2)}%`
                      : '5%',
                  }}
                ></div>
              </div>
              <div className="w-8 bg-slate-100 rounded-t relative h-full">
                ...
                {/* Pierna Derecha */}
                <div
                  className={`absolute bottom-0 w-full bg-indigo-500 transition-all duration-200 rounded-t
                            ${metrics.isRightLeg ? 'opacity-100' : 'opacity-30'}`}
                  style={{
                    height: metrics.isRightLeg
                      ? `${Math.min(100, (180 - metrics.activeHipAngle) * 2)}%`
