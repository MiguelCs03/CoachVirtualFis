...
const Home = () => {
      ...
      PlanService.verificarSesionStripe(sessionId)
        .then(async (response) => {
          if (response.plan_activated || response.payment_status === 'paid') {
            await refrescarPlan?.();
            showNotification('Plan personalizado activado con éxito. El sistema ha sido actualizado.', 'success');
  ...
  const openCreateModal = async () => {
    ...
    try {
      ...
      const byId = {};
      for (const e of list) {
        if (!byId[e.id]) { byId[e.id] = true; uniq.push(e); }
      }
  ...
  const handleCreateRoutine = async () => {
    if (!newRutinaNombre.trim()) return showNotification('El identificador de protocolo es requerido.', 'warning');
    if (selectedExercises.length === 0) return showNotification('Seleccione al menos una unidad de entrenamiento.', 'warning');
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        >
          ...
          <div className="flex items-center gap-4 text-white/20 font-mono text-[10px] tracking-widest hidden lg:flex">
             ...
             <span className="text-yellow-400/60 font-black">MIE</span> <div className="w-4 h-[1px] bg-white/10" />
             <span>JUE</span>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
           <StatCard icon={Calendar} label="SESIONES / SEMANA" value={estadisticas.entrenamientosSemanales} trend="+2" />
           <StatCard icon={Clock} label="TIEMPO_TOTAL (MIN)" value={estadisticas.minutosTotal} trend="OPTIMO" color="yellow" />
           <StatCard icon={Flame} label="ENERGÍA_ACTIVA (KCAL)" value={estadisticas.caloriasQuemadas} trend="NORMAL" />
           <StatCard icon={Trophy} label="RACHA_OPERATIVA" value={estadisticas.racha} trend="RECORD" color="yellow" />
        </div>
          ...
          >
            ...
            <div className="flex items-end justify-between h-48 gap-3">
              {datosGrafica.map((dato, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-2" onMouseEnter={() => setHoveredBar(i)} onMouseLeave={() => setHoveredBar(null)}>
                   <div className="relative w-full flex items-end justify-center h-40">
                      <motion.div 
                        ...
                        className={cx(
                          ...
                          hoveredBar === i ? "bg-yellow-400 shadow-[0_0_20px_rgba(255,230,0,0.3)]" : "bg-white/[0.03]"
                        )}
                      />
                   </div>
                   <span className={cx("text-[10px] font-mono tracking-tighter", hoveredBar === i ? "text-yellow-400" : "text-white/40")}>{dato.dia}</span>
                </div>
              ))}
            </div>
          </motion.div>
        ...
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
                ...
                <h2 className="text-xl font-black uppercase tracking-widest">BIBLIOTECA DE PROTOCOLOS</h2>
            </div>
            <div className="flex gap-2">
               <button onClick={openCreateModal} className="bg-white/5 border border-white/10 px-6 py-3 text-[10px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all">
                 ENTRADA MANUAL
               </button>
               <button onClick={() => navigate('/rutinas/crear-ia')} className="bg-yellow-400 px-6 py-3 text-[10px] font-black uppercase tracking-widest text-black hover:bg-white transition-all flex items-center gap-2">
          ...
          <AnimatePresence>
            {loadingRutinas ? (
              <div className="flex items-center gap-3 font-mono text-white/40 text-xs uppercase py-20 px-4">
                <div className="w-2 h-2 bg-yellow-400 animate-ping" /> CARGANDO DATOS DE PROTOCOLOS...
              </div>
            ) : rutinas.length === 0 ? (
               <div className="border border-white/5 bg-[#0d0d0d] p-20 flex flex-col items-center text-center space-y-6">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-2xl font-black italic uppercase italic tracking-tighter">SIN PROTOCOLOS ASIGNADOS</h3>
                    <p className="text-white/40 text-[10px] font-mono tracking-widest">INICIE UNA SINCRONIZACIÓN O CARGA MANUAL PARA COMENZAR EL ENTRENAMIENTO.</p>
                  </div>
               </div>
            ) : (
            ...
            >
              <div className="p-8 space-y-8 max-h-[90vh] overflow-y-auto custom-scrollbar">
                 <div className="flex justify-between items-start border-b border-white/5 pb-6">
                    <div>
                       <h2 className="text-3xl font-black italic uppercase italic tracking-tighter leading-none">NUEVA CARGA</h2>
                       <span className="text-[9px] text-white/40 uppercase tracking-[0.4em] font-mono">ID_PROTOCOL_INIT</span>
                    </div>
                    <button onClick={() => setShowCreateModal(false)} className="p-2 text-white/20 hover:text-white transition-colors">
                      <X className="w-6 h-6" />
                    </button>
                 </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="space-y-4">
                       <IndustrialInput label="NOMBRE_PROTOCOLO" value={newRutinaNombre} onChange={(e) => setNewRutinaNombre(e.target.value)} placeholder="EJ: UPPER_BODY_FORCE" />
                       <IndustrialInput label="TIEMPO_BASE (MIN)" type="number" value={newRutinaDuracion} onChange={(e) => setNewRutinaDuracion(e.target.value)} />
                       <div className="space-y-2">
                          <label className="text-[10px] font-black text-white/40 tracking-widest uppercase">CATEGORÍA</label>
                          <select 
                 ...
                 <div className="flex justify-end gap-4 pt-6 border-t border-white/5">
                    <button onClick={() => setShowCreateModal(false)} className="px-8 py-4 text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-white transition-all">CANCELAR</button>
                    <button onClick={handleCreateRoutine} className="bg-yellow-400 text-black px-10 py-5 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-white transition-all">CONFIRMAR SINCRONIZACIÓN</button>
                 </div>
              </div>
            </motion.div>
          </motion.div>
...
const StatCard = ({ icon: Icon, label, value, trend, color }) => (
  ...
  >
    <div className="flex items-center justify-between relative z-10">
       <div className={cx("p-3 rounded-none", color === 'yellow' ? "bg-yellow-400 text-black" : "bg-white/5 text-white/40")}>
         <Icon className="w-5 h-5" />
       </div>
       <span className="text-[10px] font-mono text-white/20 tracking-tighter">{trend}</span>
    </div>
    <div className="space-y-1 relative z-10">
       <p className="text-[9px] font-black text-white/30 tracking-[0.2em] leading-none uppercase">{label}</p>
       <p className="text-3xl font-black italic uppercase tracking-tighter text-white">{value}</p>
    </div>
    {/* Background deco */}
    {color === 'yellow' && <div className="absolute top-0 right-0 w-16 h-16 bg-yellow-400/5 blur-3xl rounded-full" />}
  </motion.div>
);
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-start">
        <div className="space-y-1">
          <span className={cx(
            ...
            rutina.categoria === 'PHYSIO' ? "bg-blue-500/10 text-blue-500" : "bg-yellow-400/10 text-yellow-400"
          )}>
            PROTOCOLO_{rutina.categoria}
          </span>
          <h3 className="text-xl font-black italic uppercase italic tracking-tighter leading-none pt-1">{rutina.nombre}</h3>
        </div>
        <div className="opacity-0 group-hover:opacity-100 transition-all flex gap-1">
           <button onClick={onDelete} className="p-1.5 text-white/20 hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4" /></button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
         <div className="bg-white/[0.03] p-3 border border-white/5">
            ...
            <p className="text-sm font-mono text-white pt-1">{rutina.ejercicios} <span className="text-[9px] text-white/20">UNIT.</span></p>
         </div>
         <div className="bg-white/[0.03] p-3 border border-white/5">
...
const ExerciseSelectItem = ({ ex, isSelected, onToggle }) => (
  <div 
    ...
    className={cx(
      ...
      isSelected ? "bg-yellow-400/10 border-yellow-400 shadow-[0_0_10px_rgba(255,230,0,0.1)]" : "bg-white/[0.02] border-white/5 hover:border-white/20"
    )}
  >
     <div className={cx(
     ...
     <div className="flex-1 min-w-0">
        <p className={cx("text-[10px] font-black uppercase tracking-tight truncate", isSelected ? "text-yellow-400" : "text-white")}>{ex.nombre}</p>
        <p className="text-[8px] font-mono text-white/40 truncate">{ex.descripcion}</p>
     </div>
     {ex.url && <img src={ex.url} alt="" className="w-10 h-8 object-cover filter grayscale opacity-50" />}
  </div>
);
