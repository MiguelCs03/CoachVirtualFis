...
const candidateUrls = (uid) => ['/mis-alertas/', `/alertas/?usuario=${uid}`, '/alertas/?mine=1']
      ...
      <div className="max-w-5xl mx-auto space-y-12">
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between border-b border-white/5 pb-10">
          <div className="space-y-4">
            <button
              onClick={handleBack}
              className="group flex items-center gap-3 text-white/40 hover:text-white transition-all uppercase font-black text-[10px] tracking-widest mb-4"
            >
          ...
          >
            <RefreshCw
              className={cx(
                'w-4 h-4 transition-transform group-hover:rotate-180 duration-500',
        ...
        <section className="bg-[#0f0f0f] border border-white/5 shadow-2xl overflow-hidden relative">
          ...
          {loading ? (
            <div className="p-24 flex flex-col items-center gap-8 bg-black/40">
              ...
              <p className="text-[10px] font-mono font-black tracking-[0.5em] text-white/20 uppercase">
                DECODIFICANDO_LOGS_SISTEMA...
              </p>
            </div>
          ) : err ? (
            <div className="p-16 flex flex-col items-center gap-6 bg-red-500/5 text-center">
              ...
              <div className="space-y-2">
                <h4 className="text-xs font-black text-red-500 tracking-[0.3em] uppercase">
                  ERROR_DE_COMUNICACIÓN
                </h4>
                <p className="text-[10px] font-mono text-red-400/60 uppercase">{err}</p>
              </div>
              <button
                onClick={load}
                className="mt-4 px-8 py-2 border border-red-500/50 text-red-500 text-[9px] font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all"
              >
                REINTENTAR_COM_SYNC
              </button>
            </div>
            ...
            <div className="p-32 text-center space-y-8 bg-black/20">
              ...
              <div className="space-y-2">
                <p className="text-[11px] font-mono font-black text-white/20 tracking-[0.5em] uppercase leading-relaxed">
                  LOGS_VACÍOS:
                  <br />
                  EL SISTEMA NO HA GENERADO ALERTAS OPERATIVAS HASTA EL MOMENTO.
                </p>
              </div>
            </div>
          ) : (
            <ul className="divide-y divide-white/5">
              {items.map((a, idx) => (
                <li
                  key={a.id}
                  className="p-10 hover:bg-white/[0.02] transition-all group relative overflow-hidden"
                >
                  ...
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 pl-12 pr-4">
                      ...
                      <div className="flex flex-wrap items-center gap-6 pt-2 border-t border-white/[0.02]">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[10px] font-mono font-black tracking-widest text-white/20 uppercase">
                            TIMESTAMP:{' '}
                            <span className="text-white/40">{fmtDateTime(a.created_at)}</span>
                          </span>
                        </div>
                    ...
                    <div className="hidden md:flex flex-col items-end gap-2">
                      <span className="text-[8px] font-black tracking-widest text-white/10 uppercase italic">
                        STATUS: UNREAD
                      </span>
