...
class Alerta extends Component {
  ...
  loadList = async () => {
    ...
    try {
      ...
      const msg =
        err?.response?.data?.detail || err?.message || 'No se pudo cargar la lista de alertas.'
  ...
  loadUsers = async () => {
    ...
    try {
      ...
      const msg = err?.response?.data?.detail || err?.message || 'No se pudo cargar usuarios.'
  ...
  editRow = (row) => {
    const usuarioId = typeof row.usuario === 'object' ? row.usuario?.id : row.usuario
    this.setState({
      form: {
        id: row.id,
        mensaje: row.mensaje,
  ...
  renderUserSelect() {
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        {errorUsers && <span className="text-yellow-200 text-xs">{errorUsers}</span>}
        {hasError && <span className="text-red-300 text-xs">{String(errorsByField.usuario)}</span>}
      </div>
    )
  }
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm text-white/90">
                  ...
                  <tbody>
                    {paged.map((row) => {
                      const userObj =
                        typeof row.usuario === 'object' ? row.usuario : usersById[row.usuario]
                      const userLabel = userObj?.email || userObj?.username || (row.usuario ?? '—')
                      return (
