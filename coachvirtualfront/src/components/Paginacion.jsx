...
export default function Paginacion({ currentPage, totalItems, pageSize = 5, onPageChange }) {
  ...
  const btnBase = 'px-3 py-1 rounded-full text-sm transition border border-white/20'
  ...
  return (
    <nav aria-label="Paginación" className="mt-4 flex items-center justify-center gap-2">
      ...
      <button className={btnGhost} onClick={() => go(page - 1)} disabled={page === 1}>
        Anterior
      </button>
      ...
      {pages.map((p) => (
        <button key={p} className={p === page ? btnActive : btnGhost} onClick={() => go(p)}>
          {p}
        </button>
      ))}
      ...
      <button className={btnGhost} onClick={() => go(page + 1)} disabled={page === totalPages}>
        Siguiente
      </button>

      <button className={btnGhost} onClick={() => go(totalPages)} disabled={page === totalPages}>
        Última
      </button>
