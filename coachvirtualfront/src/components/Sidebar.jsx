...
function PlanBadge({ planActual, PLANES }) {
  ...
  return (
    <div className={cx(
      ...
      isPremium ? "border-yellow-400 bg-yellow-400/5 shadow-[5px_5px_0px_rgba(255,230,0,0.05)]" : "border-white/20 bg-white/5"
    )}>
      <div className="flex items-center gap-3 relative z-10">
        ...
        <div className="flex flex-col">
          <span className="text-[8px] font-black text-white/40 tracking-[0.2em] leading-none">STATUS DE OPERATIVIDAD</span>
          <span className={cx(
      ...
      {/* Glow para premium */}
      {isPremium && <div className="absolute top-0 right-0 w-12 h-12 bg-yellow-400/10 blur-2xl rounded-full" />}
    </div>
  );
}
...
export default function Sidebar({ open, onClose, closeOnNavigate = false }) {
  ...
  const musclePackage = useMemo(() => isSuper ? [
  ...
  return (
    ...
    >
      <header className="px-6 py-4 flex items-center gap-3 border-b border-white/5 sticky top-0 bg-[#0d0d0d] z-20">
        ...
        <h2 className="text-[10px] font-black uppercase tracking-[0.4em] italic text-white/60">SISTEMA INTEGRAL</h2>
      </header>
      ...
      <nav className="flex-1 px-4 pb-10 space-y-6">
        <div className="space-y-4">
          ...
          <ul className="space-y-1">
            {principal.map((i) => (
              <li key={i.to}>
                <NavItem to={i.to} icon={i.icon}>{i.label}</NavItem>
              </li>
            ))}
          </ul>
        </div>
                ...
                <li>
                  ...
                  >
                    <span className="flex items-center gap-3">
                      ...
                      <span className="text-white/60 group-hover:text-white transition-colors">TESORERÍA</span>
                    </span>
                    {pagosOpen ? <ChevronDown className="w-3 h-3 text-white/20" /> : <ChevronRight className="w-3 h-3 text-white/20" />}
                  </button>
                  {pagosOpen && (
                    <ul className="mt-1 ml-4 border-l border-white/5 space-y-1">
                      {pagosPackage.map((i) => (
                        <li key={i.to}>
                          <NavItem to={i.to} icon={i.icon} sub>{i.label}</NavItem>
                        </li>
                      ))}
                    </ul>
                  )}
                ...
                <li>
                  ...
                  >
                    <span className="flex items-center gap-3">
                      ...
                      <span className="text-white/60 group-hover:text-white transition-colors">INGENIERÍA</span>
                    </span>
                    {muscleOpen ? <ChevronDown className="w-3 h-3 text-white/20" /> : <ChevronRight className="w-3 h-3 text-white/20" />}
                  </button>
                  {muscleOpen && (
                    <ul className="mt-1 ml-4 border-l border-white/5 space-y-1">
                      {musclePackage.map((i) => (
        ...
        <div className="pt-4 border-t border-white/5">
          ...
          <ul className="mt-4 space-y-1">
             <li><NavItem to="/configuracion" icon={Settings}>AJUSTES</NavItem></li>
             <li>
               <button 
                 onClick={() => {/* handle logout if needed or just use Header */}}
...
function NavItem({ to, icon: Icon, children, sub = false }) {
  return (
    ...
    >
      {({ isActive }) => (
        <>
          {isActive && <div className="absolute left-0 top-0 bottom-0 w-[3px] bg-yellow-400 shadow-[2px_0_10px_rgba(255,230,0,0.5)]" />}
          {Icon && <Icon className={cx("w-4 h-4 shrink-0 transition-colors", isActive ? "text-yellow-400" : "group-hover:text-white")} />}
