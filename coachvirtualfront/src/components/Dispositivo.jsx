  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        apply(d);
      } catch { /* Error silenciado, el polling lo manejará */ }
    })();
    ...
    <div className="fixed top-24 right-6 w-80 bg-[#0d0d0d] border border-white/10 shadow-[20px_20px_0px_rgba(255,230,0,0.03)] p-6 z-[60] font-mono">
      ...
      <div className="flex items-center justify-between mb-6 border-b border-white/5 pb-4">
        <div className="flex items-center gap-3">
            ...
            <h2 className="text-[10px] font-black tracking-[0.3em] text-white/40 uppercase">TELEMETRÍA_REMOTA</h2>
        </div>
        <button
          onClick={refreshNow}
          disabled={busy}
      ...
      {error && (
        <div className="mb-4 p-2 bg-red-500/10 border border-red-500/20 flex items-center gap-3">
            ...
            <span className="text-[8px] font-black text-red-500 tracking-widest uppercase">{error}</span>
        </div>
      )}
      ...
      {stats ? (
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex items-end justify-between">
              <span className="text-[9px] text-white/30 tracking-widest uppercase italic">UNIDADES_DESPLAZAMIENTO</span>
              <span className="text-xl font-black text-white italic tracking-tighter">{stats.steps?.toLocaleString?.() || stats.steps}</span>
            </div>
            
            <div className="relative h-1 w-full bg-white/5 overflow-hidden">
                <div className="absolute h-full bg-yellow-400 shadow-[0_0_10px_rgba(255,230,0,0.5)] transition-all duration-1000" style={{ width: pct + '%' }} />
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2 text-white/20">
                    ...
                    <span className="text-[7px] font-black tracking-widest uppercase">ENERGÍA_KCAL</span>
                </div>
          ...
          <div className="pt-4 border-t border-white/5 flex items-center justify-between">
             <div className="flex items-center gap-2">
                ...
                <span className="text-[7px] font-black text-white/10 tracking-widest uppercase italic">SYNC_TIME: {updatedAt || '--:--'}</span>
             </div>
        ...
        <div className="space-y-6">
          ...
          <p className="text-[8px] font-black text-white/10 tracking-widest uppercase text-center mt-4">BUSCANDO_CANALES_DATOS...</p>
        </div>
      )}
    </div>
  );
