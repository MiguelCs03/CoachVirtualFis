...
export class ExerciseRepCounter {
  ...
  _calculatePrimaryAngle(landmarks) {
    ...
    if (detection.wristMovement) {
      ...
      const movement =
        Math.abs(leftWrist.x - this.lastWristPos.x) + Math.abs(leftWrist.y - this.lastWristPos.y)
      this.lastWristPos = { x: leftWrist.x, y: leftWrist.y }
    ...
    if (detection.anyMovement) {
      ...
      const wristMove =
        Math.abs(leftWrist.x - this.lastPositions.wrist.x) +
        Math.abs(leftWrist.y - this.lastPositions.wrist.y)
      const elbowMove =
        Math.abs(leftElbow.x - this.lastPositions.elbow.x) +
        Math.abs(leftElbow.y - this.lastPositions.elbow.y)
      const shoulderMove =
        Math.abs(leftShoulder.x - this.lastPositions.shoulder.x) +
  ...
  _determinePhase(angle) {
    ...
    if (detection.primaryAngle) {
      const { upThreshold, downThreshold, contractedThreshold, extendedThreshold } =
        detection.primaryAngle
