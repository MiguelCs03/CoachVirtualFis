...
function construirPrompt(respuestas, ejercicios) {
  ...
  const ejerciciosFiltrados = ejercicios.todos.filter((ej) => {
    ...
    return areas.some((area) => ej.musculo.toLowerCase().includes(area.toLowerCase()))
  })
  ...
  const ejerciciosTexto = ejerciciosLimitados
    .map((ej) => `- ${ej.nombre} (ID: ${ej.id}, Músculo: ${ej.musculo}, URL: ${ej.url || ''})`)
    .join('\n')
...
function generarRutinaFallback(respuestas, ejerciciosData = null) {
  ...
  let ejerciciosDisponibles =
    ejerciciosData?.todos ||
    ejerciciosDataset.map((ej) => ({
      id: ej.id,
      nombre: ej.nombre,
    ...
    3: [
      {
        ...
        musculos: ['pecho', 'espalda', 'hombro', 'brazo', 'bíceps', 'tríceps'],
      },
