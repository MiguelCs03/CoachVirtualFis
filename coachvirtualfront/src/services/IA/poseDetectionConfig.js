  ...
  pushup: {
    checkPose: (landmarks) => {
      ...
      if (elbowAngle < 80 || elbowAngle > 100) {
        corrections.push({
          type: 'elbow',
          message: 'Los codos deben estar a 90 grados en la bajada',
        })
      }
  ...
  squat: {
    checkPose: (landmarks) => {
      ...
      if (kneeAngle > 120) {
        corrections.push({
          type: 'depth',
          message: 'Baja más, la cadera debe llegar a la altura de las rodillas',
        })
      }
      ...
      if (backAngle < 150) {
        corrections.push({
          type: 'back',
          message: 'Mantén la espalda recta, no te inclines hacia adelante',
        })
      }
  ...
  posture: {
    checkPose: (landmarks) => {
      ...
      if (nose.x > shoulder.x + 0.1) {
        corrections.push({
          type: 'head',
          message: 'Lleva la cabeza hacia atrás, está muy adelantada',
        })
      }
  ...
  glute_bridge: {
    checkPose: (landmarks) => {
      ...
      if (hip.y > knee.y) {
        corrections.push({
          type: 'hip',
          message: 'Sube más la cadera, debe estar alineada con las rodillas',
        })
      }
